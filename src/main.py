...
if __name__ == "__main__":
    while True:
        ...
        if not coord_filter.is_initialized():
            ...
            status_text = "Tracking (prediction mode)"
        else:
            status_text = "Tracking (filtered)"
            
        cv2.putText(img, f"Kalman: {status_text}", (10, 30), 
                   cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 255, 0), 2)
        
        if filtered_coords is not None and coord_filter.is_initialized():
            cv2.putText(img, f"Filtered: ({int(filtered_coords[0])}, {int(filtered_coords[1])})", 
